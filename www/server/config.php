<?php 

//Настройка БД
define('DB_HOST', 'mysql.zzz.com.ua');
define('DB_NAME', 'dimgeorgiev1976');
define('DB_USER', 'myportfolio');
define('DB_PASSWORD', 'SWQ0xK');

// SITE SETTINGS FOR EMAILS
define('SITE_NAME', 'dimdame.zzz.com.ua');
define('SITE_EMAIL', 'dim.georgiev1976@dimdame.zzz.com.ua');
define('ADMIN_EMAIL', 'dim.georgiev1976@yandex.com ');

// domcomo.000webhostapp.com

// define('SITE_NAME', 'dimig.000webhostapp.com');
// define('SITE_EMAIL', 'dimig.000@webhostapp.com');

?>